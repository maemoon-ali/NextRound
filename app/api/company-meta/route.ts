...
import { getCompanyMetaFromLiveData } from "@/lib/live-data-loader";
...
function inferDifficulty(meta: { roles_matching_filters: number; roles_at_company: number }): { level: DifficultyLevel; note: string } {
  const total = Math.max(1, meta.roles_at_company);
  const ratio = meta.roles_matching_filters / total;
  if (ratio < 0.05) return { level: "Very competitive", note: "This combination of level/function appears rarely for this company in your dataset." };
  if (ratio < 0.12) return { level: "Competitive", note: "This level/function combination is uncommon at this company in your dataset." };
  if (ratio < 0.25) return { level: "Moderate", note: "This level/function combination appears with moderate frequency at this company in your dataset." };
  return { level: "Easier", note: "This level/function combination appears frequently for this company in your dataset." };
}
...
export async function GET(request: Request) {
  ...
  const meta = getCompanyMetaFromLiveData(company, {
    function: function_ || undefined,
    level: level || undefined,
  });
  if (!meta) {
    return NextResponse.json(
      { error: "Company not found in dataset (or dataset not loaded)." },
      { status: 404 }
    );
  }
